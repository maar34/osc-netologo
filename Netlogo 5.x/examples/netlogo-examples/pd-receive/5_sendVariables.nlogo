...
;; no "port-out" defined, so it uses default parameters:
;; localHost - port: 57110

;; Example modified from: Radioactivity Decay. 
; Copyright 1997 Uri Wilensky. All rights reserved.
; The full copyright notice is in the Information tab.

;; This example shows you how to osc send any netlogo variable.
...
;; send-variables method + string + variable. 

      (osc:send-variables "decays" decays)
      (osc:send-variables "decay-chance" decay-chance)
      (osc:send-variables "last-count" last-count)
      (osc:send-variables "nuclei-number" number-nuclei)
      (osc:send-variables "nuclei" count turtles with [color = cyan])
