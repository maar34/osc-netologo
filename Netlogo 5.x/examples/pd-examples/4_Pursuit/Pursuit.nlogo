...
globals [ choice the-leader state oldstate]
...
to go
  ...
  ifelse [patch-at 1 0] of the-leader = nobody [ set state  false ] [ set state  true]
  
  (osc:send-agents "leaders" leaders "xcor" "ycor")
  (osc:send-agents "followers" followers "xcor" "ycor")
  
  if (not (state and oldstate)) [(osc:send-variables "state" state)]
  set oldstate state 
